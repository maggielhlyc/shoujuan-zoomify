...
const tilesDir = './sj_tiles'; // 指向手卷图库目录 (立轴馆可以改为 ./lz_tiles)
let artworks = [];

console.log('🤖 正在启动私人美术馆自动化排版引擎（终极三级联动版）...');

if (!fs.existsSync(tilesDir)) {
    console.error(`❌ 错误：找不到 ${tilesDir} 文件夹，请确认脚本放在与图库同级的目录下。`);
    process.exit(1);
}
...
function scanDirectory(currentPath, parentCategory = "") {
    ...
    if (isArtwork) {
        ...
        const titlePath2 = path.join(currentPath, 'title.txt.txt'); 
        ...
        let currentCatName = path.basename(currentPath); 
        ...
        if (currentPath !== tilesDir) { 
            ...
            for (let name of possibleNames) {
                ...
                const p2 = path.join(currentPath, name + '.txt'); 
                if (fs.existsSync(p1)) {
        ...
        if (currentPath === tilesDir) combinedCategory = ""; 
...
// 写入文件
fs.writeFileSync('自动化手卷图库配置.txt', jsCode, 'utf8');

console.log('\n✅ 扫描完毕，一切顺利！已为您生成【手卷图库配置.txt】。');
